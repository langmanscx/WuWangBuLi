...
@injectable()
export class ThreeSphereMouseController extends BaseMouseCOntroller implements IMouseController {
    /**
     * 可以偏移
     */
    ...
    override OnMouseMove(event: MouseEvent): void {
        ...
        if ((event.buttons === 1 && event.shiftKey) || event.buttons === 4) {
            ...
            if (this.canRotate) {
                this.rotateTo = new Point2d(event.clientX, event.clientY);
                camera.OnRotate(this.rotateFrom, this.rotateTo);
                this.rotateFrom = this.rotateTo;
            }
        } else {
