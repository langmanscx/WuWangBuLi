...
export class FloorCommand extends BaseCommand {

    public override Name: string = "Floor";

    public override Description = "生成地面";
    ...
    protected override  CreateThing(...args: (number[] | Point | IEntity[])[]): IThing[] {
        ...
        const descriptions = outers.map(x => new FloorDescription(x));
        const result = descriptions.map(x => new FloorThing(uuid4(), "地面", x));
