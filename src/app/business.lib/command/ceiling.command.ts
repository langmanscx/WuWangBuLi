...
export class CeilingCommand extends BaseCommand {

    public override Name: string = "Ceiling";

    public override Description = "生成天花板";
    ...
    protected override  CreateThing(...args: (number[] | Point | IEntity[])[]): IThing[] {
        ...
        const descriptions = outers.map(x => new CeilingDescription(x));
        const result = descriptions.map(x => new CeilingThing(uuid4(), "天花板", x));
